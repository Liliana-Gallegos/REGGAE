...
if ( opt$randsample == 0 ){ 
  ...
  # Random splitting or Full data set
    ...
    if (opt$pca != FALSE){
      ...
      # print(length(pca_sum[["center"]]))
      for (i in 2:(nrow(df_pca)-1)) ssw[i] <- sum(kmeans(df_pca,centers=i)$withinss)
      ...
      if (ks > 1) {
        ...
        for (i in 1:length(pca_sum[["sdev"]])) { 
          comp_i = pca_sum[["sdev"]][i] - pca_sum[["sdev"]][i+1]
        if (comp_i < 0.065) { n_c=i; break } }
        for (i in 2:length(pca_sum[["sdev"]])){ 
          clust_i = abs(ssw[i] - ssw[i+1])
          # print(clust_i)
          if (clust_i < 50) { n_k=i; break}} }
...
if (opt$buildmodel != FALSE){
  ...
  if (opt$model == "opt-rforest"  && y_var == TRUE) {
    ...
    if (opt$exportdata == "predicted") { 
      dfpred <- data.frame(c(data_tr$y,data_tt$y), 
                         c(round(c(model$fitted.values, predict(model, data_tt)), 2)), 
                         c(rep("Train", nrow(data_tr)), rep("Test", nrow(data_tt))))
      colnames(dfpred) <- c("Y","Pred_Y","Split")
      dfpred <- dfpred[order(dfpred$Y, decreasing = FALSE), ] 
      write.csv(dfpred, "RF_predicted-values.csv") }
...
if (opt$corrplot) {
  ...
  # num_data = select_if(df, is.numeric)  #### create corr plot with ALL data
  num_data = select_if(data_tr, is.numeric)
...
reggae_plot <- function(model) { 
  ...
  q_plot = ggplot(plot_df, aes( x = actual , y = predicted, color="Train") ) +
    ...
    scale_color_manual(values=c("#0036e6", "#999999", "#56B4E9")); q_plot
  ...
  if (opt$exportdata == "predicted") { dfpred <- data.frame(c(data_tr$y,data_tt$y), 
                         c(round(c(model$fitted.values, predict(model, data_tt)), 2)), 
                         c(rep("Train", nrow(data_tr)), rep("Test", nrow(data_tt))))
      colnames(dfpred) <- c("Y","Pred_Y","Split")
      dfpred <- dfpred[order(dfpred$Y, decreasing = FALSE), ] 
      write.csv(dfpred, "MVLR_predicted-values.csv")} } ### END Plottting ### 
...
if ( opt$verbose ){
  ...
  if (opt$buildmodel != FALSE){
    ...
    dfpred <- data.frame(c(data_tr$y,data_tt$y), 
                         ...
                         c(rep("Train", nrow(data_tr)), rep("Test", nrow(data_tt))))
    colnames(dfpred) <- c("Y","Pred_Y","Split")
    print(dfpred[order(dfpred$Y, decreasing = FALSE), ] )
    if (opt$model == "rforest") { 
      ...
      rforest_plot = ggplot(train.frame, aes( x = data$y, y = RF_predicted_tr, color="Train") ) +
        ...
        theme_bw() + theme(plot.title = element_text(hjust = 0.5), legend.title = element_blank()) +
        stat_smooth(method = "lm", col = "black") + 
        scale_color_manual(values=c("#0036e6", "#999999", "#56B4E9")) 
      if (opt$extdata != FALSE){ 
    ...
    if (opt$model == "opt-rforest") {
      ...
      rforest_plot = ggplot(train.frame, aes( x = data$y, y = RF_predicted_tr, color="Train") ) +
        ...
        theme_bw() + theme(plot.title = element_text(hjust = 0.5), legend.title = element_blank()) +
        stat_smooth(method = "lm", col = "black") + 
        scale_color_manual(values=c("#0036e6", "#999999", "#56B4E9")) #E69F00 - yellow
      if (opt$extdata != FALSE){ 
        rforest_plot_val <- rforest_plot + geom_point(data=val.frame, aes(x=data_extval$y , y=RF_predicted_val, color="Val.External"), alpha=0.7) + 
          stat_smooth(method = "lm", col = "black") +
          labs( title = paste("Random Forest Model "), 
  ...
  if ( opt$diagnostics != FALSE){
    png('REGGAE-diagnostics-plot.png')
    par(mfrow=c(2,2))
  ...
  if (opt$pca != FALSE){
    ...
    plot(1:(nrow(df_pca)-1), ssw, type="b", xlab="Number of Clusters",
